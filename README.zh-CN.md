...
- [dsh-chat-import](https://github.com/devmom/dsh-session-import) - 跨工具会话迁移：从 15+ 款 Agent（Codex / Claude Code / Kimi Code / ZCode / OpenClaw / WorkBuddy / Gemini / Cursor / Aider / Continue / OpenCode / ChatGPT / 通用 JSON）高保真导入历史会话为可续聊的 dsh 会话，保留标题与工作目录绑定，提供浏览器多选导入向导、模型工具、隐私脱敏与知识抽取（npm: chat-import）
